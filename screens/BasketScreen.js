        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <View className=" pb-96">
            {Object.entries(groupedItems).map(([key, value]) => {
              return (
                <View
                  className="flex-row justify-between items-center px-4 py-4 border-b border-gray-200 bg-white"
                  key={key}
                >
                  ...
                  <View className="flex-row justify-between items-center space-x-3">
                    <Text className="">
                      $ {(value[0].price * value.length).toFixed(2)}
                    </Text>
                    <TouchableOpacity
                      onPress={() => {
                        dispatch(removeFromBasket({ id: value[0].id }));
                      }}
                    >
        ...
        <View>
          ...
          <View className="flex-row justify-between px-5 pt-4">
            ...
            <Text className="text-gray-500">
              $ {JSON.stringify(groupedItems) === "{}" ? "0.00" : 5.99}
            </Text>
          </View>
          <View className="flex-row justify-between px-5 pt-4">
            ...
            <Text className="font-bold">
              ${" "}
              {(
                subtotal + (JSON.stringify(groupedItems) === "{}" ? 0 : 5.99)
              ).toFixed(2)}
            </Text>
          </View>
        </View>
        <TouchableOpacity
          className="mx-4 my-5 p-3 rounded-lg bg-[#00CCBB]"
          onPress={() => {
